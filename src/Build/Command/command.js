...
class CommandBase {
    constructor(commandOptions) {
        ...
        if (!Check("string", this.usage) && this.usage)
            client_1.throwErr(`The usage on the command "${this.name}" is not a typeof string.`);
        this.description = description;
        if (!Check("string", this.description) && this.description)
            client_1.throwErr(`The description on the command "${this.name}" is not a typeof string.`);
        this.category = category;
        if (!Check("string", this.category) && this.category)
            client_1.throwErr(`The category on the command "${this.name}" is not a typeof string.`);
